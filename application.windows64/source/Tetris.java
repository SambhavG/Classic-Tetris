...
public void drawTetris(int xcoord, int ycoord, int boxSize) {
  ...
  for (int i = 3; i < grid.length; i++) {
    for (int j = 0; j < grid[i].length; j++) {
      if (floor(grid[i][j]) == 1) {
        ...
        if (grid[i][j] == 1.0f) {
          resultIndex = 0;
        } else if (grid[i][j] == 1.1f) {
          resultIndex = 1;
        } else if (grid[i][j] == 1.2f) {
          resultIndex = 2;
        } else if (grid[i][j] == 1.3f) {
          resultIndex = 3;
        } else if (grid[i][j] == 1.4f) {
          resultIndex = 4;
        } else if (grid[i][j] == 1.5f) {
          resultIndex = 5;
        } else if (grid[i][j] == 1.6f) {
          resultIndex = 6;
        }
...
public int[][] findLocationsOfDrop() { //returns every point with value 2
...
public boolean canDropPiece() { //returns if piece can be moved 1 space down
...
public void dropPiece() { //drops piece 1 space down
  int[][] locations = findLocationsOfDrop();
...
public void freezePiece() { //solidifies piece
  int[][] locations = findLocationsOfDrop();
  for (int[] loc : locations) {
    grid[loc[0]][loc[1]] = 1.0f + ((float)pieceIndex/10);
  }
}
...
public void dropPieceCompletely() {
  while (canDropPiece()) {
    dropPiece();
  }
}
...
public void drawBox(int x, int y, int size, int index) {
  /*
...
public void drawStatisticsPieces() {
  int boxSize = 18;
...
public int setLevel(char key1, int adjustment) {
  if (key1 == '0') {
    return 0+adjustment;
  }
  if (key1 == '1') {
...
public void draw() {
  ...
  if ((currentMillis - lastMillis)/16.67f >= speed && !gameLost && !paused) {
